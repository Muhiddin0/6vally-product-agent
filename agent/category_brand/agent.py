...
def select_category_brand(
    ...
    # Match brand using Python logic
    ...
    system_prompt = """
You are a category selection assistant for an e-commerce platform.
Your task is to select the most appropriate category IDs based on the product name and brand name.
...
The JSON must match this structure exactly:
{
  ...
  "sub_category": "string or null (sub-category name)",
  "sub_sub_category_id": "string or null",
  "sub_sub_category": "string or null (sub-sub-category name)"
}
...
2. category_id is always required (string)
3. sub_category_id and sub_sub_category_id can be null if not available
4. Always include the category, sub_category, and sub_sub_category names when available
""".strip()
...
Select the most appropriate category IDs. Return ONLY valid JSON matching the required structure.
""".strip()
    ...
    for attempt in range(max_retries + 1):
        ...
        # Validate schema
        try:
            # Add brand_id to response data before validation
        ...
        "childes": [
            {
                ...
                "icon_full_url": {"key": "def.png", "path": None, "status": 404},
                "childes": [
                    {
                        "id": 594,
                        "name": "Кресло",
...
if __name__ == "__main__":
    ...
    # Note: BRANDS is not defined in the snippet I saw,
    # but I assume it exists further up or is intended to be there.
    # Looking at the end of the file, it seems the user had a test call.

    # I should check if BRANDS is defined.
    # Based on the previous view_file, I only saw the end of CATEGORYS.
    # I'll search for BRANDS definition first to be safe, or just wrap what was there.

    # Actually, the user had:
    # select_category_brand(
    #     categories=CATEGORYS,
    #     brands=BRANDS,
    #     brand_name="SHIVAKI",
    #     product_name="6400-G Shivaki ГП с ГК Белый",
    # )
    # So I will just wrap that.

    try:
        result = select_category_brand(
            categories=CATEGORYS,
            brands=BRANDS,
            brand_name="SHIVAKI",
            product_name="6400-G Shivaki ГП с ГК Белый",
